#!/usr/bin/env python3
"""Build and validate the CoupledMD v9 final-207 Supplementary Data package.
...
OUT = ROOT / "supplementary_data_v9_final207"
STAGE = ROOT / ".supplementary_data_v9_final207.build"
TARBALL = ROOT / "CoupledMD_v9_final207_Supplementary_Data.tar.gz"
AUDIT = ROOT / "CoupledMD_v9_final207_Supplementary_Data_audit.json"
INPUT = ROOT / "v9_figure_inputs"
COHORT = DATA / "release_cohort_v9_final207.csv"
MASTER = DATA / "systems_master.csv"
SCHEMA_VERSION = "v9-final207.1"
...
def backup_existing() -> str:
    ...
    if OUT.exists():
        target = backup / f"supplementary_data_v9_final208_{stamp}"
        if target.exists():
            raise FileExistsError(target)
        shutil.copytree(OUT, target)
    if TARBALL.exists():
        shutil.copy2(TARBALL, backup / f"CoupledMD_v9_final208_Supplementary_Data_{stamp}.tar.gz")
    return stamp
...
def build_s2(final_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for sid, reason in EXCLUDED.items():
        rows.append({
            ...
            "hold_reason": reason,
            "evidence_source": "final release decision 2026-07-14",
        })
...
def build_s8(final_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for source in read_csv(INPUT / "scidata_T8_readiness_qc_v9.csv"):
        ...
        row["included_in_final208_release"] = "True"
        rows.append(row)
    return rows
...
def build_s9(final_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for record_type, builder in portal_specs().items():
        ...
        rows.append({
            ...
            "scope_note": "current final-cohort portal/API coverage; not deposited-archive or frozen-manifest coverage",
...
def build_s10() -> list[dict[str, Any]]:
    code = [
        (ROOT / "prepare_supplementary_data_v9.py", "Supplementary Data package builder and validator"),
        (ROOT / "build_v9_figure_inputs.py", "Versioned figure-input builder"),
        (ROOT / "build_final208_structural_selection_audit_v9.py", "Structural-selection audit builder"),
        (ROOT / "build_final624_core_interface_validation_v9.py", "Replica-level technical-validation builder"),
        (ROOT / "build_final624_replica_qc_v9.py", "Replica readiness/QC ledger builder"),
        (ROOT / "plotting_scripts/final208_figures.py", "Main Figure 1-6 plotting script"),
...
DEFINITIONS = {
    ...
    "force_field": "Force-field/protocol label recorded in final system metadata; not an exact engine-version claim.",
    "lipid_composition": "Membrane lipid-composition label in final system metadata.",
    "structural_provenance": "Release label describing experimental or engineered structural provenance.",
    "trajectory_type": "Trajectory content/type label in final system metadata.",
    ...
    "included_in_final208_release": "Whether the row belongs to the authoritative final-207 cohort.",
    "expected_systems": "Number of final systems to which the portal/API record type applies.",
...
VOCABULARIES = {
    ...
    "release_selection_status": "selected_validated", "included_in_final208_release": "True",
...
def build_s3(directory: Path) -> list[dict[str, Any]]:
    ...
    rows = []
    source_groups = {
        "S1": "final cohort, systems_master.csv and per-system index.json",
        "S2": "readiness overlay and final release decision",
        ...
        "S7": "final cohort replica-selection rule",
        "S8": "v9 readiness/QC evidence overlay",
...
def validate_package(directory: Path) -> dict[str, Any]:
    ...
    assert {row["system_id"] for row in tables["S2"] if row["release_status"] == "excluded"} == EXCLUDED
    ...
    assert not final_derived_ids & (EXPECTED_UNRESOLVED | EXCLUDED)
...
def build(directory: Path) -> None:
    ...
    (directory / "README.txt").write_text(
        "CoupledMD v9 final-207 Supplementary Data\n\n"
