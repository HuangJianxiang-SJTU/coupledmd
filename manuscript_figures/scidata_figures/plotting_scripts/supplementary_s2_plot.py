...
OUT = ROOT / "publication_figures_final208"
...
def main() -> None:
    figure = build_figure()
    stem = OUT / "supplementary_figure_s2_gateway_method"
    ...
    # Keep existing manuscript/build references resolving to the corrected S2.
    for suffix in (".pdf", ".png"):
        shutil.copyfile(
            stem.with_suffix(suffix),
            OUT / f"supplementary_figure_s2_server_coverage{suffix}",
        )

    for path in sorted(OUT.glob("supplementary_figure_s2_*")):
        print(path)
