...
(B) Membrane environment. 212 of 222 systems were simulated in a POPC lipid bilayer; 10 protein-only systems were simulated in explicit solvent without membrane.
...
(A) Pocket Jaccard similarity versus gateway dissimilarity for all 16 receptor contrasts with multi-partner data. OX2R (Gq versus Gs) falls at Jaccard = 0.0 — the most extreme pocket-landscape reorganization in the dataset.
(B) Pairwise pocket-residue Jaccard similarity among OX2R's three G-protein complexes. The Gq↔Gs comparison is 0.00, i.e. complete non-overlap of pocket-lining residues, whereas Gi↔Gq and Gi↔Gs each retain 0.50.
(C) Heatmap of gateway open fraction across all seven TM portal pairs for OX2R under Gi, Gq, and Gs coupling, illustrating how the lipid-accessible interface is remodelled by partner identity.
(D) Pocket Jaccard similarity versus α5 tilt difference for the 13 contrasts with both descriptors, OX2R highlighted. Across these contrasts, pocket divergence and α5 geometry difference show no significant association (Spearman, n = 13), indicating that pocket-landscape reorganization is not simply a consequence of α5 re-tilting.
