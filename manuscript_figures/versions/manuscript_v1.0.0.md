# CoupledMD: a web resource for comparative molecular dynamics of 222 GPCR–G-protein ternary complexes reveals dynamic coupling selectivity
...
All 222 systems were parameterized with the CHARMM36m force field [6] and simulated in three independent replicas of 500 ns, yielding approximately 332 μs of aggregate sampling (Figure 1C). Of the systems, 186 were executed with the AMBER pmemd engine [9] using chamber-converted topologies, 10 protein-only systems with GROMACS [8], and 26 class-B receptor systems prepared via CHARMM-GUI [10]. A total of 212 systems were embedded in a POPC lipid bilayer; the remaining 10 were simulated in explicit solvent without membrane. Of the 222 structures, 211 derive from experimental coordinates; 11 originate from engineered or chimeric constructs (BRIL-fused receptors, recovered identifiers) and are flagged in the database. The top 15 receptor genes by system count span class A aminergic, peptide, and lipid-sensing receptors (Figure 1D).

### 3.2 Pocket landscape and allosteric site recovery

Transient binding pockets at the intracellular face of each receptor were identified using fpocket [11] with a consensus filter requiring detection in at least two of three replicas. Of 222 systems, 209 yield consensus pocket data; the 13 exceptions correspond to simulations where the intracellular cavity remained occluded throughout all replicas. Lining residues were mapped to GPCRdb generic positions [12] to enable cross-receptor alignment and clustering. Consensus clustering across the full database identifies 50 druggable pocket clusters spanning 182 unique receptors, with per-cluster mean occupancy frequencies ranging from 0.864 to 0.927.

As a prospective validation, we examined CCR5 (system Gi_7F1Q; Figure 2). Consensus cluster 50, centred on residues at GPCRdb positions 2.50, 3.39, and 7.49, corresponds to the Na⁺ allosteric pocket and the adjacent maraviroc binding site [13, 14]—the binding site of an FDA-approved allosteric CCR5 inhibitor used to treat HIV infection. This pocket is recovered with a mean occupancy frequency of 0.927 from MD dynamics alone, without structural input from the inhibitor-bound conformation (Figure 2A, B). The recovery of a clinically validated allosteric site through unbiased MD sampling demonstrates that the CoupledMD pipeline captures functionally relevant transient cavities that could serve as starting points for allosteric drug discovery in receptor systems where no allosteric ligand is yet known.

The uniform simulation protocol of CoupledMD uniquely enables direct comparison of pocket landscapes between G-protein partners of the same receptor. Across 16 receptor contrasts with MD data for two or more G-protein partners, Jaccard similarity coefficients for pocket residue sets range from complete overlap (1.0) to complete non-overlap (0.0). The most extreme case is OX2R, which is coupled to Gi, Gq, and Gs in independent simulations: the Gq–Gs contrast yields Jaccard = 0.0, indicating that the intracellular pocket landscape is entirely reorganized upon partner switching (Figure 5). Receptor contrasts such as EDNRB (Gi–Gq, Jaccard = 0.6) and GHSR (Gi–Gq, Jaccard = 0.67) show intermediate divergence, suggesting a spectrum of pocket sensitivity to G-protein identity. Because all paired simulations in CoupledMD share an identical force field and analysis pipeline, these Jaccard differences reflect biological coupling selectivity rather than methodological artefact.

### 3.3 G-protein coupling geometry and interface barcodes

Per-position root-mean-square fluctuation (RMSF) and contact persistence were computed for the G-protein α-subunit at all 356 CGN positions [2]. The CGN system classifies these positions into four functional categories: conserved (142), selectivity-determining (127), paralog-specific (69), and neutral (18). Contact persistence—the fraction of simulation frames in which a receptor–G-protein residue pair falls within a 4 Å heavy-atom distance cutoff, averaged across replicas—constitutes a per-system G-protein barcode directly comparable across all 222 simulations (Figure 4A). Order parameters (S²) and χ₁ rotameric entropy were computed where applicable, providing additional measures of side-chain flexibility at the coupling interface.

Four geometric descriptors characterize the G-protein α5 helix quantitatively: tilt angle relative to the receptor transmembrane axis (range 29.8°–73.1°, mean 52.9°), insertion depth into the intracellular cavity (range 20.4–50.8 Å, mean 32.2 Å), solvent-accessible surface area of the buried α5 tip, and hook angle at the C-terminal terminus (range 1.1°–12.3°). A composite functional rank integrating all four descriptors stratifies systems by coupling interface compactness (Figure 4B); OX2R ranks highest, consistent with its extreme partner-switching behaviour.
...
**Partner-switching analysis.** Receptors with MD data available for two or more G-protein subfamily partners were identified (16 receptor contrasts). For each contrast, pocket residue sets were compared using the Jaccard similarity coefficient, and gateway distance distributions were contrasted across partners. OX2R, with data for Gi, Gq, and Gs coupling, shows Jaccard = 0.0 between Gq and Gs, representing the most extreme case of pocket-landscape reorganization upon partner switching in the database.

**Allosteric site recovery.** As prospective validation, CCR5 (Gi_7F1Q) was examined. Consensus cluster 50, centred on residues at generic positions 2.50, 3.39, and 7.49, corresponds to the Na⁺ allosteric pocket and the adjacent maraviroc binding site [13, 14]. Recovery of this pocket from MD dynamics without structural input from the inhibitor-bound form demonstrates that the database captures functionally relevant transient cavities.
...
(B) Membrane representation. 212 of 222 systems were simulated in a POPC lipid bilayer; 10 protein-only systems were simulated in explicit solvent without membrane.
...
(D) Top 15 most-represented receptor genes, coloured by predominant G-protein subfamily. Class A aminergic and peptide receptors dominate, reflecting the current experimental ternary complex structural coverage.

**Figure 2. Prospective allosteric site recovery in CCR5.**
(A) Mean pocket occupancy frequency for all 13 consensus pockets in CCR5 (Gi_7F1Q), ranked by frequency. Red bars: orthosteric site; blue bars: non-orthosteric pockets. The dashed vertical line marks consensus cluster 50 (the Na⁺/maraviroc allosteric site; mean frequency 0.927), recovered blind from MD without an inhibitor-bound starting structure.
(B) Pocket size (lining residue count) versus mean occupancy frequency for all 13 pockets. Bubble area is proportional to pocket volume. The top non-orthosteric pocket (cluster 50) is annotated; residues at GPCRdb positions 2.50, 3.39, and 7.49 constitute the Na⁺ binding motif co-localising with the maraviroc binding site.
(C) Summary of the CCR5 case study, including system identifier, pocket frequency, key generic residue positions, and cross-reference to the clinically approved allosteric site.
...
(A) Gateway occupancy (fraction of frames in the open state) for all seven transmembrane portal pairs, shown side-by-side for Gi (blue) and Gq (red) with 95% confidence intervals. The TM6–TM7 portal shows the largest divergence (Gi: 0.022 vs Gq: 0.208; ~ten-fold), indicating near-complete closure under Gi coupling.
(B) Mean gateway distance (Å) for all seven portal pairs. The dashed line at 8 Å denotes the empirical open-state threshold. Partner-dependent divergence is localized to specific portals rather than uniformly distributed across the interface.

**Figure 4. G-protein coupling interface barcode and α5 geometry landscape.**
(A) Contact persistence barcode for an exemplar system (CCR5–Gi), with each bar representing one of the 356 CGN positions, coloured by mean contact persistence (white = 0, teal = 1.0) and grouped by CGN segment. Selectivity-determining positions (Flock classification) are marked above.
(B) Scatter plot of α5 tilt angle versus insertion depth across 27 systems for which both descriptors are available, coloured by G-protein subfamily. OX2R (Gs coupling, PDB 7L1V) ranks highest by composite functional rank and is annotated.
(C) Cross-family contact persistence heatmap: mean per-CGN-segment persistence averaged within each G-protein subfamily, revealing conserved versus family-specific interface regions.
(D) RMSF profile along the G.H5 segment across all 222 systems, grouped by subfamily. The C-terminal tip of α5 (G.H5 positions 24–26) shows the largest variance across systems, consistent with its role as the primary selectivity-encoding region.
...
(A) Jaccard similarity coefficient versus gateway distance divergence for all 16 receptor contrasts with multi-partner data. OX2R (Gq vs Gs) falls at Jaccard = 0.0—the most extreme pocket-landscape reorganization in the dataset.
(B) Side-by-side pocket occupancy bar charts for OX2R–Gq (Gq_7SR8) and OX2R–Gs, demonstrating complete non-overlap of detected pockets between partners.
(C) Heatmap of gateway occupancy across all seven TM portal pairs for OX2R under Gi, Gq, and Gs coupling, illustrating how the lipid-accessible interface is remodelled by partner identity.
(D) Scatter of Jaccard similarity versus composite α5 geometry difference for all 16 contrasts, with OX2R highlighted. Systems with extreme pocket divergence also tend to show larger α5 geometry differences, suggesting a structural basis for coupling-dependent pocket reorganization.
